...
public:
	...
	const MatrixElementList & Elements() const
	{
		return elements;
	}
	...
	// solve the system Ax = b by using Bi-Conjugete Gradient Method
	// with pre-factoring the matrix A
	void BCG_ATA(double* b, double* x, int maxIter, double tolerance)
	{
		int iter;
		double errNew;
		double errOld;
		...
		for (int i=0; i<n; i++) d2[i] = d[i] = r2[i] = r[i] = b[i] - r[i];
		errNew = 0;
		for (int i=0; i<n; i++) errNew += r2[i] * r[i];
		err = abs(errNew);
		...
		while (iter<maxIter && abs(errNew) > tolerance*err)
		{
			...
			double alpha = 0;
			for (int i=0; i<n; i++) alpha += d2[i] * q[i];
			alpha = errNew / alpha;
			...
			if (iter % 50 == 0)
			{
				...
				Multiply(d2, tmp); PreMultiply(tmp, q);
				for (int i=0; i<n; i++) r2[i] -= alpha * q[i];
				cout << errNew << " " << tolerance*err << endl;
			}
			else
			{
			...
			for (int i=0; i<n; i++) d2[i] = r2[i] + beta * d2[i];
			iter++;
		}
