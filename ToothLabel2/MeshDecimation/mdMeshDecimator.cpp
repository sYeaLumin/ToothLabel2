...
namespace MeshDecimation
{
	...
	void MeshDecimator::Initialize(size_t nVertices, size_t nTriangles, size_t paraSize, Vec3<Float> * points, Vec3<int> * triangles, int * labels, int * m_sLevel, float * m_para)
    {
		...
		this->m_sLevel.resize(m_nVertices, 0);
		if (labels != NULL && m_sLevel != NULL)
		{
			for (size_t v = 0; v < m_nVertices; ++v)
			{
				this->m_sLevel[v] = m_sLevel[v];
			}
		}
		this->m_para.clear();
		for (size_t i = 0; i < paraSize; i++)
		{
			this->m_para.push_back(m_para[i]);
		}
    }
    ...
    void MeshDecimator::GetMeshData(Vec3<Float> * points, Vec3<int> * triangles, int * labels) const
    {
    ...
    double MeshDecimator::ComputeEdgeCost(int v1, int v2, Vec3<Float> & newPos) const
    {
		...
		if (m_sLevel.size()>0)
		{
			int level = std::max(m_sLevel[v1], m_sLevel[v2]);
			if (level<(int)m_para.size())
				qem *= m_para[level];	
		}
        return qem;
    }
