...
static bool read_ply(FILE *f, TriMesh *mesh)
{
	...
	while (LINE_IS("obj_info") || LINE_IS("comment")) {
		if (LINE_IS("obj_info num_cols"))
			sscanf_s(buf, "obj_info num_cols %d", &mesh->grid_width);
		if (LINE_IS("obj_info num_rows"))
			sscanf_s(buf, "obj_info num_rows %d", &mesh->grid_height);
		GET_LINE();
	}
	...
	while (!LINE_IS("end_header") && !LINE_IS("element vertex")) {
		...
		sscanf_s(buf, "element %s %d", elem_name, &nelem);
		GET_LINE();
	...
	result = sscanf_s(buf, "element vertex %d\n", &nverts);
	if (result != 1) {
	...
	if (LINE_IS("element face")) {
		if (sscanf_s(buf, "element face %d\n", &nfaces) != 1)
			return false;
		GET_LINE();
		while (LINE_IS("property")) {
		...
		if (sscanf_s(buf, "element range_grid %d\n", &ngrid) != 1)
			return false;
...
static bool read_ray(FILE *f, TriMesh *mesh)
{
	while (!feof(f)) {
		...
		if (LINE_IS("#vertex")) {
			...
			int f1, f2, f3, m;
			if (fscanf_s(f, "%d %d %d %d", &m, &f1, &f2, &f3) != 4) {
				return false;
			}
...
static bool read_obj(FILE *f, TriMesh *mesh)
{
	...
	while (1) {
		...
		if (LINE_IS("v ") || LINE_IS("v\t")) {
			float x, y, z;
			if (sscanf_s(buf+1, "%f %f %f", &x, &y, &z) != 3) {
				return false;
			}
			mesh->vertices.push_back(point(x,y,z));
...
static bool read_off(FILE *f, TriMesh *mesh)
{
	...
	if (sscanf_s(buf, "%d %d %d", &nverts, &nfaces, &unused) < 2)
		return false;
	...
	for (int i = old_nverts; i < new_nverts; i++) {
		for (int j = 0; j < vert_len; j++) {
			if (j == vert_pos) {
				...
				float r, g, b;
				if (fscanf_s(f, "%f %f %f", &r, &g, &b) != 3)
					return false;
				mesh->colors[i] = Color(r,g,b);
				j += 2;
			} else if (j == vert_color && !float_color) {
				int r, g, b;
				if (fscanf_s(f, "%d %d %d", &r, &g, &b) != 3)
					return false;
				mesh->colors[i] = Color(r,g,b);
				j += 2;
			} else if (j == vert_conf) {
				if (fscanf_s(f, "%f", &mesh->confidences[i]) != 1)
					return false;
			} else {
				fscanf_s(f, " %1024s", buf);
			}
...
static bool read_strips_asc(FILE *f, TriMesh *mesh)
{
	...
	skip_comments(f);
	for (int i = old_striplen; i < new_striplen; i++)
		if (fscanf_s(f, "%d", &mesh->tstrips[i]) != 1)
			return false;
	...
	bool write_norm, bool float_color)
{
	...
	strcpy_s(meshname, strlen(meshname),filename);
